...
public interface Joins {
    ...
    @SafeVarargs
    static <A> BiFunction<CriteriaBuilder, From<A, A>, Join<A, ?>[]> of(BiFunction<CriteriaBuilder, From<A, A>, Join<A, ?>>... joins) {
        return (cb, from) -> {
            return Arrays.stream(joins).map(j -> j.apply(cb, from)).toArray(Join[]::new);
        };
    }

    static <A, B> BiFunction<CriteriaBuilder, From<A, A>, Join<A, ?>> join(SingularAttribute<? super A, B> attr, JoinType type,
    ...
    static <A, B> BiFunction<CriteriaBuilder, From<A, A>, Join<A, ?>> inner(SingularAttribute<? super A, B> attr,
    ...
    static <A, B> BiFunction<CriteriaBuilder, From<A, A>, Join<A, ?>> left(SingularAttribute<? super A, B> attr,
            BiFunction<CriteriaBuilder, Path<? extends B>, Predicate[]> predicates) {
